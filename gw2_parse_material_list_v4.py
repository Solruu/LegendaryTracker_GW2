...
Usage :
    python3 gw2_parse_material_list_v4.py                 # toutes les pages
    python3 gw2_parse_material_list_v4.py bolt frostfang  # une selection
    python3 gw2_parse_material_list_v4.py --json out.json
"""
import json
import re
import sys
...
def aretes(chemin):
    ...
    for rang in _grille(html[j:k]):
        ...
        if not tete or not enfant or tete == enfant:
            continue
        # v4 : la deduplication ne vaut QUE pour l'arete de tete, et jamais
        # pour les sous-items d'une meme cellule.
        #
        # Une cellule qui couvre plusieurs lignes est repetee ligne par ligne :
        # « Mystic Tribute -> Gift of Condensed Might, 2 » sort quatre fois sur
        # la page de Vision, une par sous-don, et qui somme lit 8 pour 2. C'est
        # le meme fait, on le garde une fois.
        #
        # Mais dans la troisieme colonne d'UNE SEULE ligne, le meme ingredient
        # peut figurer deux fois, et ce sont alors deux exigences distinctes :
        # le Gift of Research demande 250 reactifs hydrocatalytiques PUIS encore
        # 250, soit les 500 que sa boite Recipe affiche. La v3 dedupliquait tout
        # et ramenait ce poste a 250 — elle corrigeait un defaut en en creant un
        # autre, plus discret.
        if (tete, enfant, q) not in vus_tete:
            vus_tete.add((tete, enfant, q))

...
const makeGateStatus = (acct) => (gate) => {
  ...
  return gate.map((g) => {
    ...
    if (g?.type === "mastery") {
      // L'API rend des identifiants numeriques, pas des noms. Sans table de
      // correspondance, on sait seulement si le scope repond : on ne pretend
      // pas trancher une maitrise qu'on ne sait pas nommer.
      return {
        open: acct?.masteries_scope_ok ? null : null,
        label: NX({ fr: `Maîtrise « ${g.name} »`, en: `'${g.name}' mastery` })
          + (acct && acct.masteries_scope_ok === false
             ? NX({ fr: " — scope « progression » absent de la clé", en: " — key lacks the 'progression' scope" })
             : ""),
      };
    }
    if (g?.type === "expansion") {
...
export default function GW2LegendaryTracker() {
  ...
  const buildDirectProgressionData = useCallback(async (key) => {
    ...
    const msOk = rms.ok;
    const msList = msOk ? await rms.json().catch(() => null) : null;
    const _gates = {

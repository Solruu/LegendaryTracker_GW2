...
export default function GW2LegendaryTracker() {
  ...
  useEffect(() => {
    ...
    const cacheKey = `gw2_ach_bits_${selectedLeg}_${lang}_v5`; // v5 : requirement/description + retry subs
    try {
      const cached = JSON.parse(localStorage.getItem(cacheKey) ?? "null");
    ...
    (async () => {
      try {
        ...
        const out = {};
        for (const d of defs) out[String(d.id)] = { bits: d.bits ?? [], names, req: d.requirement ?? "", desc: d.description ?? "", lockedTxt: d.locked_text ?? "" };
                    ...
                    {!a.recipe && def && (def.bits.length === 0 ? (
                      ...
                      return (
                        <div key={i} style={{ padding: "2px 0" }}>
                          ...
                          {!stepDone && a.bitTips?.[i] && (
                            <div style={{ margin: "1px 0 3px 18px", fontSize: "10px", fontFamily: "'Crimson Text', serif", color: "rgba(226,201,126,0.42)", lineHeight: 1.45 }}>{NX(a.bitTips[i])}</div>
                          )}
                        </div>
                      );
                    }))}

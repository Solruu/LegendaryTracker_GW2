...
- **Un rendement quotidien est une cadence, pas un onglet.** Les monnaies de
  carte (rubis de sang, baies de houx, perles orriennes…) portent leur plafond
  dans `cadence.sources[]`, avec `per_character: true` quand il se démultiplie
  par personnage. Aucun onglet ne recalcule ce rendement dans son coin.
- **L'onglet Guide ne contient aucune donnée** : il rend `legendaries[].guide`.
...
- **Dans `qty`, la clé décide du sens.** Une clé qui désigne un **composant**
  est une quantité **par parent**, que la cascade multiplie. Une clé qui désigne
  un **légendaire** est un **total déjà aplati**, lu tel quel. Même syntaxe,
  deux sens — c'est l'origine des erreurs de facteur de ce projet (Dragonite
  250 au lieu de 100, reliques d'Ad Infinitum ×7, Branded Mass 300 au lieu de
...
- **Une porte inconnue n'est pas une porte fermée.** `_gates` (palier de
  ...
  alors « ❔ » et laisse la collection visible. Masquer une collection jouable
  coûte des semaines, en afficher une de trop coûte une ligne.
- **Les deux chemins de synchro doivent rendre la même chose.** Flask
...
- **Une exigence de matériau transverse ne vit que dans `qty`.** `computeGrandTotal`
  ne lit que `craft_components[].qty` ; une valeur écrite seulement dans
  `_meta.common_required` s'affiche en référence et **compte pour zéro**. C'est
...
- **Ce qui attend une vérification vit dans `BACKLOG.md`**, pas dans un
  ...
  **en l'état** tant qu'il y figure : on ne devine pas à la place d'une source.
  ...
  erreurs.
- **Avant tout push** : `python3 gw2_audit_v13.py`. Un échec bloque le push.
...
- **Un identifiant faux ne provoque jamais d'erreur.** Il renvoie un stock nul,
  ...
  portant un autre id est faux à coup sûr. `gw2_audit_v13.py` applique cette règle.
...
- **La question qui manque presque toujours : est-ce gratuit et répétable ?** Un
  ...
  sur le composant, et `gw2_audit_v13.py` signale les gros postes qui n'en ont
  aucune.
...
- **Une donnée écrite n'est pas une donnée affichée.** Trois fois de suite, une
  ...
  champ est réellement lu par un chemin de rendu. `gw2_audit_v13.py` liste

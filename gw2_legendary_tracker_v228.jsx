...
const I18N = {
  en: {
    ...
    wb_req_note: "Requires WvW rank 350 + Commander's Compendium (300g + 250 Badges) for the last Warcry tier. Minimum 6 weeks (2450 tickets at 365+90/week).",
    ach_locked: "Locked",
  ...
  fr: {
    ...
    wb_req_note: "Requiert rang McM 350 + Commander's Compendium (300 po + 250 Badges) pour le dernier palier de Warcry. Minimum 6 semaines (2450 tickets à 365+90/sem).",
...
const LEGENDARIES = {
  vision: {
    ...
    resetType: "daily",
    currencies: [
      // Les six monnaies de carte de LW4. Elles manquaient : l'onglet Composants
      // n'affichait qu'elegie, gemmes et magie volatile, alors qu'Aurora y liste
      // depuis toujours les six monnaies de LW3. Les couts etaient dans l'arbre
      // et dans le grand total, mais invisibles la ou on les cherche — d'ou
      // l'impression de les decouvrir en jeu. Chaque `required` est le total que
  ...
  warbringer: {
    ...
    requirements: {
      ...
      lines: [
        { icon: "🎟", label: { fr: "Skirmish Claim Tickets", en: "Skirmish Claim Tickets" }, perUnit: 2450, curKey: "tickets",
...
function WaypointList({ items, isDone, copied, onCopy, orderLabel, orderTitle }) {
  return (
    <div>
      {items.map((item, i) => {
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                ...
                {item.wp_order && orderLabel && (
                  <span title={orderTitle}
                    style={{ fontSize: 9, color: "rgba(251,220,80,0.75)", background: "rgba(251,220,80,0.07)", border: "1px solid rgba(251,220,80,0.22)", borderRadius: 3, padding: "1px 5px" }}>
...
function AchatsUniques({ totals, NX, acquises, setAcquises, recettesCompte, recetteParDon, commander }) {
  const cc = SOURCES_DB?.craft_components ?? {};
  const utiles = Object.entries(cc)
    ...
    .map(([cid, c]) => ({ cid, nom: c.name, don: cc[c.enseigne]?.name ?? c.enseigne,
                          ...
                          auto: (() => {
                            // Le tag de commandant se lit sur le compte, pas
                            // dans les recettes : il n'en est pas une.
                            if (cid === "commanders_compendium") return commander === true;
...
// Lien wiki d'une etape, indexe par (succes, numero de case).
//
// `NomEtape` ne servait que les trois rendus d'Aurora, qui lisent leurs etapes
// dans SOURCES_DB. Le rendu GENERIQUE — celui de Vision et de toutes les autres
// cibles — construit ses libelles depuis la definition de succes de l'API, ou
// `item.wiki` n'existe pas. Les 2 399 liens poses le 18/09 n'y apparaissaient
// donc jamais : non pas casses, jamais branches.
//
// D'ou cette table, batie une fois : (id de succes, numero de case) -> page.
const WIKI_PAR_BIT = new Map();
// Meme table pour les NOMS. Le rendu generique tire ses libelles de la
// definition de succes de l'API ; quand elle n'en donne pas, il retombait sur
// « STEP 1 », « STEP 2 »… alors que nos sources portent le vrai nom de chaque
// case. Chef-Tonnerre affichait sept « STEP n » cliquables, ce qui ne dit rien
// de ce qu'on va ouvrir.
const NOM_PAR_BIT = new Map();
(() => {
  for (const leg of Object.values(SOURCES_DB?.legendaries ?? {})) {
    for (const col of Object.values(leg?.collections ?? {})) {
      if (!col?.id) continue;
      for (const it of (col.items ?? [])) {
        if (typeof it?.bit !== "number") continue;
        const cle = `${col.id}-${it.bit}`;
        if (it.wiki) WIKI_PAR_BIT.set(cle, it.wiki);
        if (it.name) NOM_PAR_BIT.set(cle, it.name);
      }
    }
  }
})();

function LienWiki({ page, children }) {
  if (!page) return children;
  return (
    <a href={`https://wiki.guildwars2.com/wiki/${page}`} target="_blank" rel="noreferrer"
       title="wiki" onClick={e => e.stopPropagation()}
...
function NomEtape({ item, NX }) {
  return <LienWiki page={item.wiki}>{NX(item.name)}</LienWiki>;
}
...
function computeGrandTotal(selectedIds, collectionsByLeg) {
  ...
  // l'audit le verifie.
  // Une etape de collection DEJA VALIDEE qui pointe vers un composant rend ce
  // composant inutile : il est fabrique, il ne reste rien a farmer, ni lui ni
  // ce qui le compose. Six armes Astral terminees, et les 3 000 minerais de
  // kralkatite comme les 3 000 poudres de quartz rose disparaissent du total.
  //
  ...
  //
  // La regle est generale : elle vaut pour toute etape portant `component`,
  // sur n'importe quelle cible. Le moteur Python applique la meme, et le test
  // de conformite compare les deux.
  ...
  for (let pass = 0; pass < 6; pass++) {
    ...
    for (const [gid, g] of Object.entries(altGroups)) {
      for (const t of (g.targets ?? [])) {
        ...
        if (!n) continue;
        const opt = altPick(gid, t);
        if (opt && !satisfaits.has(opt)) add[opt] = (add[opt] ?? 0) + (g.qty ?? 0) * n;
      }
    }
    let changed = false;
...
// legendaire (une seule cible). Meme lecture/ecriture localStorage que les
// autres onglets independants (cf. computeGrandTotal) : chaque montage relit
// l'instantane persiste plutot que de faire descendre l'etat par props sur
// plusieurs niveaux — un choix fait dans un onglet est donc vu par l'autre des
// son prochain montage (changement d'onglet), sans etat partage a cabler.
function AltGroupsPicker({ selectedIds = [], totals = {} }) {
  const cc = SOURCES_DB?.craft_components ?? {};
  const C = "#e2c97e", D = "rgba(226,201,126,";
  ...
  const groupes = Object.entries(ALT_GROUPS)
...
function GrandTotalTab({ ownedIds = new Set(), manualOwnedIds = new Set(), onToggleManual, stocks = {}, onSetStockManual }) {
  ...
  const rows = Object.entries(totals)
    ...
    .sort((a, b) => {
      // Niveau 1 : à farmer avant déjà acquis (stock connu et suffisant).
      // Niveau 2 : alphabétique sur le nom (toujours une chaîne simple,
      // meme nom technique quelle que soit la langue de l'UI).
      const acquisA = a.owned !== null && a.missing === 0;
      const acquisB = b.owned !== null && b.missing === 0;
      if (acquisA !== acquisB) return acquisA ? 1 : -1;
...
export default function GW2LegendaryTracker() {
  ...
  const buildDirectProgressionData = useCallback(async (key) => {
    ...
    let recipesDirect = [];
    try {
      const rr = await fetch(`https://api.guildwars2.com/v2/account/recipes?${tk}`);
      if (rr.ok) { const j = await rr.json(); if (Array.isArray(j)) recipesDirect = j; }
    } catch (_) { /* la detection retombe en manuel */ }
  ...
  const fetchFromFlask = useCallback(async () => {
    ...
    try {
      ...
      // Quel que soit le chemin — Flask ou direct — les recettes du compte
      // alimentent la detection des feuilles achetees.
      if (Array.isArray(data._recipes) && data._recipes.length) {
        setRecettesCompte(new Set(data._recipes));
      }
      // Le compendium de commandant n'est pas une recette : /v2/recipes/search
  ...
  const detectGtArmory = useCallback(async (keyOverride) => {
    ...
    try {
      ...
      const flaskResp = await fetch(
        ...
        // 3 s etait beaucoup trop court : cet appel traverse Flask JUSQU'A
        // l'API GW2 (gw2_get a lui-meme 10 s de timeout), et /api/progression
        // s'accorde 120 s pour le meme genre de trajet -- dans le log fourni
        // il mettait 11 s. Un compte un peu lent depassait donc les 3 s, le
        // fetch etait avorte, et l'abort tombait dans le catch "Flask absent".
        { ...(key ? { headers: { "X-API-Key": key } } : {}), signal: AbortSignal.timeout(30000) }
      );
      ...
      // Ce catch rapportait toujours "Flask absent". Or il attrape aussi
      // l'abort du timeout ci-dessus et toute erreur reseau/CORS -- trois
      // causes distinctes, un seul message, et donc un diagnostic faux des
      // que Flask repond mais lentement. On garde la cause.
      flaskErr = (e && e.name === "TimeoutError")
        ? "Flask n'a pas répondu en 30 s (appel GW2 trop lent ?)"
        : (e && e.name === "AbortError")
          ? "requête interrompue"
          : null; // vraie absence de serveur : on laisse le message historique
    }
    ...
    if (data === null) {
      // Ni Flask (pas actif, ou actif mais sans cle -- ni tapee ni en .env cote
      // serveur) ni appel direct (pas de cle tapee) n'ont pu repondre. Le dire
  ...
  // apiId -> identifiant de composant, pour relier une ligne du bloc
  // `currencies` a ce que le moteur en dit.
  const compByApi = React.useMemo(() => {
    const m = new Map();
    for (const [cid, c] of Object.entries(SOURCES_DB?.craft_components ?? {})) {
      if (typeof c?.apiId === "number" && !m.has(c.apiId)) m.set(c.apiId, cid);
    }
    return m;
  }, []);

  // La progression des collections vit dans DEUX etats : `auroraCollections`
  // pour la plupart des cibles, `visionCollections` pour l'onglet Vision. Le
  // moteur ne lisait que le premier — donc cocher « Vision of Equipment:
  // Astral Weapons » ne retirait pas les 3 000 minerais de kralkatite, alors
  // que la regle du 18/09 etait en place et correcte. Les deux espaces de cles
  // sont disjoints (`vis_*` d'un cote), la fusion est sans ambiguite.
  const toutesCollections = React.useMemo(
    () => ({ ...(auroraCollections ?? {}), ...(visionCollections ?? {}) }),
    [auroraCollections, visionCollections]);
  ...
  const wpnCurrencies = React.useMemo(() => {
    ...
    let totals = {};
    try { totals = computeGrandTotal(wpnLegIds, toutesCollections)?.totals ?? {}; }
    catch (_) { return []; }
    return (leg?.currencyCatalog ?? [])
      .map(c => ({ ...c, required: Math.round(totals[c.comp] ?? 0) }))
      .filter(c => c.required > 0);
  }, [isWeapons, wpnLegIds.join(","), toutesCollections, leg]);
  ...
  const legCurrencies = isGrandTotal ? [] : (isArmorSet
    ...
    : (isWeapons
      ? wpnCurrencies
      : (leg?.currencies ?? []).map(c => {
          // v218 : le `required` vient du MOTEUR, plus du nombre ecrit a la
          // main dans le bloc `currencies`.
          //
          // Sans cela, la regle « une etape validee satisfait son composant »
          // ne pouvait pas atteindre cet affichage : Antoine avait termine les
          // six armes Astral et lisait toujours « Kralkatite Ore, requis
          // 3 100 » alors que le moteur, lui, disait 100. Le grand total
          // obeissait a la regle, la colonne par legendaire non.
          //
            ...
            <div style={{ margin: "10px 14px 0" }}>
              ...
              {auroraSubExpanded === "empowering" && (
                <div style={{ borderLeft: "1px solid rgba(52,211,153,0.2)", borderRight: "1px solid rgba(52,211,153,0.2)", padding: "4px 13px 10px 13px", background: "rgba(255,255,255,0.01)" }}>
                  {/* Meme liste, meme composant. Ce bloc en etait une copie
                      manuelle : memes champs, meme mise en forme, mais son etat
                      se lisait a l'index (`bits2.has(i)`) au lieu du `bit` que
                      chaque etape porte. Deux rendus pour une liste, c'est
                      ainsi qu'ils divergent -- Vision II l'a montre. */}
                  <WaypointList items={sourcesAurora?.collections?.aurora_2?.items ?? []}
                    copied={copiedCode} onCopy={setCopiedCode}
                    orderLabel={n => t("wp_order", { n })} orderTitle={t("wp_order_title")}
                    isDone={i => {
                      const it = (sourcesAurora?.collections?.aurora_2?.items ?? [])[i];
                      return ach2Done || bits2.has(it?.bit ?? i);
                    }} />
                </div>
              )}
                ...
                {isOpen && (
                  <div style={{ marginTop: "8px", borderTop: "1px solid rgba(226,201,126,0.08)", paddingTop: "7px" }} onClick={e => e.stopPropagation()}>
                    ...
                    {(listeLieux && listeLieux.items.length >= def.bits.length) ? null : def.bits.map((b, i) => {
                      ...
                      // Nos sources avant le repli « STEP n » : elles portent
                      // le vrai nom de la case quand l'API n'en donne pas.
                      if (!label) {
                        const n = NOM_PAR_BIT.get(`${a.achievementId}-${i}`);
                        if (n) label = NX(n);
                      }
                      let unnamed = false;
                      ...
                      return (
                        <div key={i} style={{ padding: "2px 0" }}>
                          <div style={{ display: "flex", alignItems: "baseline", gap: "6px", fontSize: "11px", color: stepDone ? "#4ade80" : "rgba(226,201,126,0.6)" }}>
                            ...
                            <span style={{ textDecoration: stepDone ? "line-through" : "none", opacity: stepDone ? 0.7 : 1 }}>
                              <LienWiki page={WIKI_PAR_BIT.get(`${a.achievementId}-${i}`)}>{label}</LienWiki>
                            </span>
